# task-service-runtime Specification

## Purpose

The runtime/platform concerns of task-service: local RS256 JWT validation using the auth-service public key without any per-request call to auth (NFR-2), authentication enforcement on protected endpoints, the health endpoint, the standardized error envelope, CORS for the frontend origin, env-driven configuration, and a containerized service with its own `taskdb` database started from the root `docker-compose.yml`. Covers AC-4 (validation side) and extends one-command startup (AC-15).
