# task-cards Specification

## Purpose

The kanban card domain owned by task-service: the `cards` data model and its CRUD/mutation API, plus the role/ownership authorization rules (SPEC §4.2) — ADMIN sees and edits all cards; a USER sees cards they created OR are assigned, and may edit/delete only cards they created; USER creates auto-assign self; ADMIN may assign any user. Covers AC-5…AC-12.
