# stack-integration Specification

## Purpose

The full system starts healthy with a single `docker compose up --build`: the
containerized frontend (nginx, port 3000), task-service, auth-service, task-db, and
auth-db come up together, the SPA is served from nginx (not the host dev server) and
works end-to-end against the containerized backends, and the repository ships the NFR-6
run + architecture documentation. Closes SPEC AC-15.
