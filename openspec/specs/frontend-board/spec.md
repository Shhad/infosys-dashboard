...
target column, via task-service `PATCH /api/cards/{id}/status`. The UI SHALL reflect
the new column on success and revert/surface the error on failure (e.g. `403`).

#### Scenario: Permitted move updates column
- **GIVEN** a user permitted to change a card's status
- **WHEN** they move the card to another column
...
`DELETE /api/cards/{id}`, removing it from the board on success.
